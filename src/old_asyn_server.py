...
async def handle(conn, addr): ## recv_send  between clinets, one turn of data transfer+ result back
    ...
    while True:
        ...
        if data_len and file_name:
            ...
            logger_prefix = 'azure-multi-infer_log_server_' + str(args['request_rate']) + 'rps_' + ' train_' + args[
...
async def work(ip,port): # multi thread processing
    global cnt
    print(ip,port)
...
if __name__ =='__main__':
    ...
    ip, port = '0.0.0.0', 54100
    asyncio.run(work(ip, port))